...
| #7 complete transforms | `blockToCanvas` = full `Placement` (origin + scale + rotation about frame centre). Overlay transform = source-pixel→frame sizing + origin + user scale/rotation. |
...
| 4 | `placementMatrix` applies the frame origin as the outermost translation, so a non-zero origin produces translation even with scale=1/rotation=0. |
...
| X3 | **Authored-timing skips** — a row whose selected variant has a matte source that is timing-inactive at a chosen frame time is a real authored outcome; it is deterministically SKIPPED (typed `unsupportedLayerMode("matte source is timing-inactive"/"hidden")`), NOT a failure to force. Every enumerated row is accounted for: `realRowCount + skippedRowCount == enumeratedRowCount`. Any other compile error is a STOP (`rowCompileFailed`). |
