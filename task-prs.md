...
- **Locator**: `ProjectMediaLocator.absoluteURL(for: MediaRef, registry: ProjectAssetRegistry) async throws -> URL` — canonical, value-passed, instance-scoped. Legacy single-arg API is a protocol-extension bridge only, marked deprecated, and has zero production call sites in `Sources/` (grep-enforced).
