...
final class ScenePlayerDiagnosticTests: XCTestCase {
    ...
    func testDiagnostic_sceneLevel() async throws {
    ...
    func testDiagnostic_perFrameSummary() async throws {
        let (package, animations) = try loadTestPackage()
        try await MainActor.run {
            let player = ScenePlayer()
            let compiled = try player.compile(package: package, loadedAnimations: animations)
    ...
    func testDiagnostic_blockLevelScope() async throws {
        let (package, animations) = try loadTestPackage()
        try await MainActor.run {
            let player = ScenePlayer()
            let compiled = try player.compile(package: package, loadedAnimations: animations)
    ...
    func testDiagnostic_effectiveMatrixAndScissor() async throws {
        let (package, animations) = try loadTestPackage()
        try await MainActor.run {
            let player = ScenePlayer()
            let compiled = try player.compile(package: package, loadedAnimations: animations)
    ...
    func testDiagnostic_fullReport() async throws {
        print("\n")
        print(String(repeating: "#", count: 80))
        print("# FULL DIAGNOSTIC REPORT")
        print(String(repeating: "#", count: 80))

        try await testDiagnostic_sceneLevel()
        try await testDiagnostic_perFrameSummary()
        try await testDiagnostic_blockLevelScope()
        try await testDiagnostic_effectiveMatrixAndScissor()
