...
final class VariantSwitchTests: XCTestCase {
    ...
    func testCompilation_block01HasTwoVariants() async throws {
    ...
    func testCompilation_block02HasOneVariant() async throws {
        try await MainActor.run {
            let player = try makePlayer()
            let compiled = player.compiledScene!
    ...
    func testAvailableVariants_returnsCorrectVariantInfo() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testAvailableVariants_unknownBlock_returnsEmpty() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testSelectedVariantId_defaultIsFirstVariant() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testSelectedVariantId_unknownBlock_returnsNil() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testSetSelectedVariant_changesActiveVariant() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testVariantSwitch_changesDrawImageAssetId() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testVariantSwitch_doesNotAffectOtherBlocks() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testEditMode_alwaysUsesNoAnimVariant() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testInvalidVariant_fallsBackToDefault() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testInvalidVariant_removesExistingOverride() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testApplyVariantSelection_setsMultipleBlocks() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testApplyVariantSelection_skipsInvalidEntries() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testClearOverride_revertsToDefault() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testCommandsBalanced_afterVariantSwitch() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testDeterminism_sameVariantSameFrame() async throws {
        try await MainActor.run {
            let player = try makePlayer()
            player.setSelectedVariant(blockId: "block_01", variantId: "v2")
    ...
    func testMergedAssetIndex_containsAllVariantAssets() async throws {
        try await MainActor.run {
            let player = try makePlayer()
            let compiled = player.compiledScene!
    ...
    func testPreviewAnimVariant_hasInputClipFromEditVariant() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testEditMode_hasInputClip() async throws {
        try await MainActor.run {
            let player = try makePlayer()
    ...
    func testBeforeCompile_apiReturnsDefaults() async {
        await MainActor.run {
            let player = ScenePlayer()
